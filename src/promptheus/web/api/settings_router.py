...
# Serialize validate_api_key calls that temporarily mutate os.environ
...
async def update_settings(update: SettingsUpdate, request: Request):
    ...
    try:
        ...
        response_value = masked_value if "API_KEY" in update.key else update.value
        return {"success": True, "key": update.key, "value": response_value}
